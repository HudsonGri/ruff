...
impl<'a, 'c, 'db> TypeRelationChecker<'a, 'c, 'db> {
    ...
    pub(super) fn check_type_pair(
        ...
        match (source, target) {
            ...
            // For assignability, a bare ParamSpec type variable is compatible with a gradual
            // ParamSpec value (`...` or `Any` in a ParamSpec slot). This is required for generic
            // wrappers like `Command[**P, T]` to widen to `Command[..., object]`.
            (Type::TypeVar(bound_typevar), _)
                if self.relation.is_assignability()
                    && !bound_typevar.is_inferable(db, self.inferable)
                    && bound_typevar.is_paramspec(db)
                    && is_gradual_paramspec_value(db, target) =>
            {
                self.always()
            }
            ...
            //
            // Bare ParamSpec type variables and gradual ParamSpec values are mutually assignable.
            (_, Type::TypeVar(bound_typevar))
                if self.relation.is_assignability()
                    && !bound_typevar.is_inferable(db, self.inferable)
                    && bound_typevar.is_paramspec(db)
