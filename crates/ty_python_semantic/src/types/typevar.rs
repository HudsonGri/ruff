            ...
            None => {
                let inferred_variance = match self.binding_context(db) {
                    BindingContext::Definition(definition) => {
                        ...
                        binding_ty.as_type_alias().map_or_else(
                            || binding_ty.with_polarity(polarity).variance_of(db, self),
                            |type_alias| {
                ...
                match inferred_variance {
                    // Bivariance is only the raw inferred result. Effective variance falls back to
                    // covariance so unused parameters don't become mutually assignable.
