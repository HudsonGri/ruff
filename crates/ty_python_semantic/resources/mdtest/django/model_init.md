# Django Model __init__ Synthesis
...
## Model can be instantiated with correct keyword arguments

Django synthesizes an `__init__` that accepts each field as a keyword argument with the appropriate
type. Passing correctly typed values should not produce any diagnostics.
...
e = Event(name="Concert", count=100)
```

## Auto-generated id field is optional in __init__

The implicit `id` primary key is an `AutoField` whose value is assigned by the database on save. It
should not be required when constructing a new instance.
