...
## ForeignKey reverse accessor uses default \_set name

When model `Comment` has `ForeignKey(Post, ...)`, Django synthesizes a `comment_set` reverse
accessor on `Post` that returns a manager for querying related `Comment` instances.
...
class Comment(Model):
    post = ForeignKey(Post, on_delete=None)

p = Post()
reveal_type(p.comment_set)  # revealed: Manager[Comment]
```

## ForeignKey reverse accessor with custom related_name

When `related_name="comments"` is specified on the ForeignKey, Django uses that name instead of the
default `{model}_set` on the target model.
...
class Comment(Model):
    post = ForeignKey(Post, on_delete=None, related_name="comments")

p = Post()
reveal_type(p.comments)  # revealed: Manager[Comment]
```

## OneToOneField reverse returns the related model instance type

When `Profile` has a `OneToOneField(User, ...)`, the reverse accessor on `User` (named `profile` by
default, without the `_set` suffix) returns an instance of `Profile` directly (not a manager), since
there can be at most one related object.
...
## Nullable OneToOneField reverse includes None in the type

When the `OneToOneField` uses `null=True`, the reverse accessor may return `None` if no related
object exists, so the type is `Profile | None`. The accessor name follows the same `{model}` default
convention (no `_set` suffix for OneToOneField).
...
class Profile(Model):
    user = OneToOneField(User, on_delete=None, null=True)

u = User()
reveal_type(u.profile)  # revealed: Profile | None
```

## ForeignKey reverse accessor works across files

The reverse-relation index scans all first-party modules, so a reverse accessor is synthesized even
when the source and target models live in separate files.
