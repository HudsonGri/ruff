# Django ForeignKey \_id Accessor Synthesis
...
## ForeignKey synthesizes a <fieldname>\_id accessor of type int

When a model defines `author = ForeignKey(User, ...)`, Django synthesizes an `author_id` attribute
that holds the raw integer primary key of the related object (without performing a database join).
...
class Post(Model):
    author = ForeignKey(User, on_delete=None)

p = Post()
reveal_type(p.author_id)  # revealed: int
reveal_type(p.author)  # revealed: User
```

## Nullable ForeignKey gives int | None for \_id accessor

When `null=True` is specified on the ForeignKey, the synthesized `_id` attribute becomes
`int | None` because the foreign key column can be NULL.
...
class Comment(Model):
    user = ForeignKey(User, null=True, on_delete=None)

c = Comment()
reveal_type(c.user_id)  # revealed: int | None
```
